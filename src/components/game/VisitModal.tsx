...
export function VisitModal({
  ...
  const earnPreview = Math.floor(
    visit.basePay *
      (selectedGirl
        ? 1 + (selectedGirl.beauty + selectedGirl.performance + selectedGirl.popularity) / 220
        : 1) *
      ...
      >
        ...
        <div className="grid gap-4 p-3 lg:grid-cols-2">
            ...
            <div className="grid grid-cols-1 gap-1.5">
              {VISIT_TYPES.map((v) => {
                ...
                return (
                  <button
                    key={v.id}
                    onClick={() => !vLocked && setVisitId(v.id)}
                    disabled={vLocked}
            ...
            >
              <div className="flex items-baseline justify-between">
                <span className="font-bold text-accent">
                  🛋️ Oppgrader trailer-tilbud → Lv {state.trailerLevel + 1}
                </span>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              ...
              {state.girls.length === 0 && (
                <span className="text-[10px] text-muted-foreground">Ingen stjerner i staben.</span>
              )}
              {state.girls.map((g) => {
          ...
          <div className="rounded-md border border-border bg-background/40 p-2 text-[11px]">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-mono">
                +{heat.total} (base {heat.baseApplied}
                {heat.intensityBonus > 0 ? ` + ${heat.intensityBonus}` : ""})
              </span>
            </div>
            <div className="mt-1 rounded border border-border/70 bg-card/40 px-2 py-1.5 text-[10px]">
              ...
              <div className="mt-1 flex items-center justify-between">
                ...
                <span>
                  -{staminaCost} stamina · {visit.hours}t
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span>Valgt stjerne</span>
                <span>
                  {selectedGirl ? `Opptatt ca. ${cooldownHours}t etterpå` : "Ingen ekstra cooldown"}
                </span>
              </div>
              {(visit.risky || std) && (
                <div
                  className={`mt-1 ${visit.risky && intensity === "intense" ? "text-amber-300" : "text-muted-foreground"}`}
                >
                  {visit.risky
                    ? intensity === "intense"
                      ? `🧪 Risikorull aktiv (condoms: ${state.condoms}).`
                      : "🧪 Ingen risikorull før du skrur opp intensitet."
                    : "STD påvirker utbetaling/tilgang."}
                </div>
              )}
            </div>
            {std && (
              <div className="mt-1 text-[10px] text-destructive">
                {std.emoji} {selectedGirl!.name} har {std.name} — påvirker payout og kan blokkere
                VIP.
              </div>
            )}
          ...
          <button
            ...
            onClick={() => {
              onRun(visit.id, girlId, intensity);
              onClose();
            }}
...
function Tag({
  ...
  return (
    <span
      className={`rounded px-1 py-0.5 font-bold uppercase tracking-wider ${
        hot
