...
export function ProductionsSheet({
  state,
  onClose,
  onStart,
  onAdvance,
      ...
      >
        ...
        <div className="mt-3 rounded-lg border border-border bg-secondary/30 p-3">
          ...
          <div className="mt-2 rounded-md border border-dashed border-border bg-background/40 p-2 text-[10px] text-muted-foreground">
            🛒 Oppgrader utstyret hos{" "}
            <span className="text-foreground">📷 Sparky's Camera Shack</span>. Filmstock fra
            Sparky's, kostymer fra <span className="text-foreground">👗 Glitter & Garter</span>,
            audition-vouchers fra <span className="text-foreground">🎟️ Open Mic Casting</span>.
          </div>
          <div className="mt-2 grid grid-cols-3 gap-2 text-[10px]">
            <span className="rounded bg-background/60 px-2 py-1 text-center">
              🎞️ {state.filmstock}
            </span>
            <span className="rounded bg-background/60 px-2 py-1 text-center">
              👗 {state.costumes}
            ...
            <span className="rounded bg-background/60 px-2 py-1 text-center">
              🎟️ {state.auditionVouchers}
            </span>
          </div>
        </div>

        {/* New project */}
        <h3 className="mt-4 font-display text-sm uppercase tracking-widest text-accent">
          Start nytt prosjekt
        </h3>
        ...
        <div className="mt-2 rounded-lg border border-border bg-secondary/30 p-2">
              ...
              return (
                ...
                >
                  <div className="flex items-baseline justify-between">
                    ...
                    <span className="font-mono text-accent text-[9px]">
                      ★{fans} ×{fanMult.toFixed(2)}
                    </span>
                  </div>
          ...
          <div className="mt-1 text-[10px] text-muted-foreground">
            Valgt: <span className="text-foreground">{getGenre(genreId)?.name}</span> —{" "}
            {getGenre(genreId)?.blurb}
          </div>
        </div>
                ...
                <div className="relative p-2.5">
                  ...
                  <div className="mt-1 text-[10px] text-muted-foreground">
                    {locked
                      ...
                      : full
                        ? "🚫 Kø full"
                        : `Briefing: $${cost} · ${totalHours}t total`}
                  </div>
                </div>
              </button>
            );
...
function ProductionCard({
  ...
  const nextStage =
    !isDone && p.stageIdx < STAGE_ORDER.length - 1 ? tier.stages[p.stageIdx + 1] : null;
  const canAdvance = !isDone && p.hoursLeft <= 0;
  const stageCostToAdvance =
    canAdvance && p.stageIdx < STAGE_ORDER.length - 1 ? stageCost(nextStage!, mods) : 0;
  ...
  const stageLabel = isDone
    ? "Ferdig"
    : `${currentStage?.label ?? "Ukjent"}${p.hoursLeft > 0 ? ` · ${p.hoursLeft}t igjen` : " · klar"}`;
        ...
        <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-2 p-2">
          <div>
            ...
            <p className="text-[10px] uppercase tracking-wider text-accent drop-shadow">
              {tier.name}{" "}
              {p.genreId
                ? `· ${getGenre(p.genreId)?.emoji ?? ""} ${getGenre(p.genreId)?.name ?? ""} `
                : ""}
              · Q{Math.round(p.quality)}/{mods.qualityCap}
          ...
          {isDone ? (
            ...
            <button
              ...
              className="rounded bg-destructive/70 px-2 py-0.5 text-[10px] text-destructive-foreground hover:bg-destructive"
            >
              Avlys
            </button>
          )}
        ...
        <div className="mt-2 grid gap-1 rounded-md border border-border/60 bg-background/40 p-2 text-[11px]">
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-foreground">
              ${forecast.conservativeGross.toLocaleString()}–$
              {forecast.expectedGross.toLocaleString()}{" "}
              <span className="ml-1 rounded bg-primary/20 px-1 py-0.5 text-[9px] uppercase text-primary">
                Estimate
              </span>
            </span>
          </div>
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-foreground">
              {Math.round(forecast.flopChance * 100)}%{" "}
              <span className="ml-1 rounded bg-primary/20 px-1 py-0.5 text-[9px] uppercase text-primary">
                Estimate
              </span>
            </span>
          ...
          <details className="mt-2 rounded-md border border-border/60 bg-background/40 p-2 text-[11px]">
            ...
            <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-muted-foreground">
              ...
              <div className="font-mono text-foreground">
                +${lastResult.moneyDelta.toLocaleString()}
              </div>
              <div>Rep</div>
              <div className="font-mono text-foreground">
                {lastResult.reputationDelta >= 0 ? "+" : ""}
                {lastResult.reputationDelta}
              </div>
              <div>Final kvalitet</div>
              <div className="font-mono text-foreground">Q{lastResult.finalQuality}</div>
              <div>Resultat</div>
              <div className="font-mono text-foreground">
                {lastResult.flopped ? "Flopp" : "Hit"}
              </div>
              <div>Genre-match effekt</div>
              <div className="font-mono text-foreground">
                ×{lastResult.genreMatchEffect.toFixed(2)}
              </div>
              <div>Kampanje brukt</div>
              <div className="font-mono text-foreground">+{lastResult.campaignBonusConsumed}%</div>
              <div>Distrib brukt</div>
              <div className="font-mono text-foreground">+{lastResult.distribBonusConsumed}%</div>
              <div>Fans per genre</div>
              <div className="font-mono text-foreground">
                {Object.entries(lastResult.fanGainByGenre).length
                  ? Object.entries(lastResult.fanGainByGenre)
                      .map(([gid, gain]) => `${getGenre(gid)?.emoji ?? ""}${gid}+${gain}`)
                      .join(" · ")
                  : "—"}
              </div>
            </div>
              ...
              {girls.map((g) => {
                ...
                return (
                  <button
                    ...
                    title={
                      g.mission
                        ? `Opptatt: ${g.mission.label}`
                        : `${g.name} · ${suggestRole(g).reason}`
                    }
                  ...
                  return (
                    <div key={gid} className="flex items-center gap-2">
                      ...
                      <span
                        ...
                        className="rounded bg-primary/20 px-1 py-0.5 text-[9px] uppercase text-primary"
                      >
                        {suggestion.role}
                      </span>
                        ...
                        {CAST_ROLES.map((r) => {
                          ...
                          return (
                            <button
                              ...
                              title={
                                past
                                  ? `${r.label} – steget er ferdig`
                                  : `${r.hint} · ${suggestion.reason} · score ${suggestion.scores[r.id]}`
                              }
                ...
                <div className="flex flex-wrap gap-1">
                  {state.distributionDeals.map((deal) => {
                    const eligible =
                      ...
                      Math.round(Math.max(0, Math.min(mods.qualityCap, p.quality))) >=
                        deal.minQuality &&
                    ...
                    return (
                      <button
                        key={deal.id}
                        disabled={!eligible}
                        onClick={() => onAssignDeal(p.id, deal.id)}
