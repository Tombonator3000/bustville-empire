...
export function WebcamModal({
  ...
  const upgradeCost = state.webcamLevel < WEBCAM_SHOWS.length ? WEBCAM_UPGRADE_COST(state.webcamLevel) : 0;
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-background/80 backdrop-blur-sm p-3" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div className="flex items-start justify-between border-b border-border bg-secondary/40 p-3">
          <div>
            <div className="text-[10px] uppercase tracking-widest text-accent">Trailer · Webcam-rigg</div>
            <h3 className="font-display text-2xl uppercase neon-text">💻 Velg Show</h3>
            <p className="text-[10px] text-muted-foreground">Rigg-nivå {state.webcamLevel}/{WEBCAM_SHOWS.length}</p>
          </div>
        ...
        <div className="space-y-3 p-3">
          {/* Show types */}
          <div>
            <div className="mb-1 text-[10px] uppercase tracking-wider text-muted-foreground">Type show</div>
                ...
                return (
                  <button key={s.id} onClick={() => !sLocked && setShowId(s.id)}
                    ...
                    <div className="flex-1 py-2 pr-3">
                      ...
                      {sLocked && (
                        <div className="text-[10px] font-bold text-destructive">🔒 Krever webcam-rigg Lv {s.level}</div>
                      )}
                    </div>
                  </button>
                );
          ...
          {state.webcamLevel < WEBCAM_SHOWS.length && (
            <button onClick={onUpgrade}
              ...
              <div className="flex items-baseline justify-between">
                <span className="font-bold text-accent">📡 Oppgrader webcam-rigg → Lv {state.webcamLevel + 1}</span>
          ...
          {/* Girl picker */}
          <div>
            <div className="mb-1 text-[10px] uppercase tracking-wider text-muted-foreground">Hvem opptrer?</div>
            <div className="flex flex-wrap gap-1">
              ...
              {state.girls.length === 0 && <span className="text-[10px] text-muted-foreground">Ingen stjerner.</span>}
              {state.girls.map((g) => {
          ...
          <button
            ...
            onClick={() => { onRun(show.id, girlId, intensity); onClose(); }}
            ...
            {locked ? "🔒 Låst" : selectedBlocked ? "⛔ Velg ledig stjerne" : `${show.emoji} Start ${show.label}`}
          </button>
        </div>
      </div>
    </div>
