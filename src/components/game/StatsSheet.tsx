...
export function StatsSheet({
  ...
  return (
    <div
      className="fixed inset-0 z-40 flex justify-start bg-background/70 backdrop-blur-sm"
      onClick={onClose}
    >
      <div
      ...
      >
        ...
        <div className="mt-2 space-y-1.5">
          {state.weeklyRivalSummary.rivals.map((r) => (
            <div key={r.id} className="rounded-md border border-border bg-secondary/30 p-2">
              ...
              <div className="mt-1 grid grid-cols-3 gap-1 text-[11px]">
                <span title="Market share moves from PR wins, legal pressure, and stronger distribution.">
                  Share <b>{r.sharePct}%</b>
                </span>
                <span>Weekly action</span>
                <span title="Positive means they gained momentum this week.">
                  Δ{" "}
                  <b>
                    {r.deltaPct > 0 ? "+" : ""}
          ...
          {!!state.weeklyRivalSummary.effects.length && (
            <div className="rounded bg-secondary/40 p-2 text-[11px] text-muted-foreground">
              {state.weeklyRivalSummary.effects.map((effect, idx) => (
                <p key={`${idx}-${effect}`}>{effect}</p>
              ))}
            </div>
          )}
          <p
            className="text-[11px] text-muted-foreground"
            title="Your share grows when rivals lose share."
          >
            Player share trend: <b>{state.weeklyRivalSummary.playerMarketSharePct}%</b> (week{" "}
            {state.weeklyRivalSummary.week})
          </p>
        </div>
        ...
        <div className="mt-2 space-y-1.5">
          {state.distributionSummary.map((deal) => (
            <div
              key={deal.id}
              className="rounded border border-border bg-secondary/30 p-2 text-[11px]"
            >
              <p className="font-semibold">
                {deal.label} · {deal.royaltyPct}%
              </p>
              <p className="text-muted-foreground">{deal.terms}</p>
              <p title="Weeks remaining only decreases while deal is active.">
                {deal.isActive ? `Active · ${deal.weeksRemaining}w left` : "Inactive"}
              </p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-2 rounded border border-border bg-secondary/30 p-2 text-[11px]">
          ...
          <p>
            Catalog base: <b>${state.weeklyRoyaltyBreakdown.baseCatalogPayout}</b>
          </p>
          <p>
            Deal royalties: <b>${state.weeklyRoyaltyBreakdown.dealPayoutTotal}</b>
          </p>
          {state.weeklyRoyaltyBreakdown.byTitle.map((row) => (
            <p key={`${row.title}-${row.dealLabel}`} className="text-muted-foreground">
              {row.title} via {row.dealLabel}: +${row.payout} ({row.royaltyPct}%)
            </p>
          ))}
        </div>
      </div>
