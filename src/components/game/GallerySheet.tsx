      ...
      >
          ...
          <div className="grid grid-cols-2 gap-2 overflow-y-auto px-3 pb-3">
            ...
            {stats.map(({ girl, photos, dates }) => {
              ...
              return (
                <button
                  key={girl.id}
                  onClick={() => setSelectedId(girl.id)}
                  className={`group flex flex-col items-center rounded-lg border p-2 transition ${
                    on
                      ? "border-primary bg-primary/15"
                      : "border-transparent hover:border-primary/40 hover:bg-background/30"
                  }`}
                >
                  <div
                    className={`relative h-16 w-16 overflow-hidden rounded-full ring-2 ${on ? "ring-primary" : "ring-border"}`}
                  >
                    <img
                      src={ARCHETYPE_PORTRAITS[girl.archetype]}
                      alt={girl.name}
        ...
        <section className="relative flex flex-1 flex-col overflow-hidden">
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, oklch(0.35 0.12 30 / 0.55), oklch(0.18 0.10 290 / 0.65))",
            }}
          />
          <div
            ...
            style={{
              backgroundImage:
                "radial-gradient(ellipse at 70% 30%, oklch(0.7 0.18 30 / 0.6), transparent 60%)",
            }}
          />
          ...
          <div className="relative flex items-center justify-between border-b border-border/60 bg-background/40 px-5 py-3 backdrop-blur">
            <div className="flex gap-6 text-sm font-bold uppercase tracking-wider">
              <TabBtn on={tab === "all"} onClick={() => setTab("all")}>
                Alle
              </TabBtn>
              <TabBtn on={tab === "dates"} onClick={() => setTab("dates")}>
                Dates{" "}
                <span className="ml-1 font-mono text-xs text-muted-foreground">
                  {totals.d}/{stats.length * 4 || 0}
                </span>
              </TabBtn>
              <TabBtn on={tab === "photos"} onClick={() => setTab("photos")}>
                Photos{" "}
                <span className="ml-1 font-mono text-xs text-muted-foreground">
                  {totals.p}/{stats.length * MAX_PHOTOS_PER_GIRL || 0}
                </span>
              </TabBtn>
            </div>
            <button onClick={onClose} className="rounded bg-secondary px-3 py-1 text-xs">
              ✕ Lukk
            </button>
          </div>
          ...
          >
            ...
            <div className="p-4">
              ...
              <div className="text-xs text-muted-foreground">
                {preview.girl.name} · {preview.girl.archetype} · dag {preview.scene.day} ·{" "}
                {preview.scene.kind}
              </div>
              <button
                onClick={() => setPreview(null)}
                className="mt-3 w-full rounded bg-primary px-3 py-1.5 text-sm font-bold text-primary-foreground"
              >
                Lukk
              </button>
            </div>
...
function TabBtn({
  ...
  return (
    ...
    >
      {children}
      <span
        className={`absolute -bottom-0.5 left-0 right-0 h-[2px] rounded-full transition ${on ? "bg-primary" : "bg-transparent"}`}
      />
    </button>
  );
}
  ...
  return (
    <div className="absolute inset-0">
      ...
      <div
        ...
        style={{
          background: `linear-gradient(135deg, oklch(0.55 0.18 ${scene.hue} / 0.55), oklch(0.2 0.1 ${(scene.hue + 60) % 360} / 0.55))`,
        }}
      />
      ...
      <div
        className={`absolute ${large ? "right-3 bottom-3 h-12 w-12" : "right-1.5 bottom-1.5 h-7 w-7"} overflow-hidden rounded-full ring-2 ring-background/80`}
      >
