...
export function RecruitRevealModal({ girl, onClose }: { girl: Girl; onClose: () => void }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[120] bg-gradient-to-br from-indigo-950 via-purple-950 to-slate-950/95 p-4"
      onClick={onClose}
    >
      <div
        className="relative mx-auto flex h-full w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-indigo-300/20 bg-black/20 p-6 shadow-[0_0_80px_rgba(192,132,252,0.25)] md:grid md:grid-cols-2"
      ...
      >
        ...
        <section className="z-10 flex flex-col justify-center gap-3 md:pr-6">
          ...
          <p className="text-xl text-yellow-200">
            {"★".repeat(starRating)}
            <span className="ml-3 rounded-full bg-fuchsia-500/20 px-3 py-1 text-xs uppercase tracking-wider text-fuchsia-100">
              {rarity}
            </span>
          </p>
          <div className="space-y-1 text-sm text-indigo-100/90">
            <p>
              <span className="text-indigo-300">Age:</span> {age}
            </p>
            <p>
              <span className="text-indigo-300">Archetype:</span> {girl.archetype}
            </p>
            <p>
              <span className="text-indigo-300">Profession:</span> {profession}
            </p>
            <p>
              <span className="text-indigo-300">Prefers:</span> {preferences.join(" • ")}
            </p>
            <p>
              <span className="text-indigo-300">Salary:</span> ${girl.salary}/week
            </p>
            {girl.hiddenPotential ? (
              <p>
                <span className="text-indigo-300">Potential:</span>{" "}
                {POTENTIAL_LABELS[girl.hiddenPotential] ?? "Hidden Potential"}
              </p>
            ) : null}
          </div>
          <div className="flex flex-wrap gap-2">
            {preferences.map((tag) => (
              <span
                key={tag}
                className="rounded-full bg-pink-500/20 px-2.5 py-1 text-xs text-pink-100"
              >
                {tag}
              </span>
            ))}
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {[
              ...
              ["Popularity", girl.popularity],
              ["Loyalty", girl.loyalty],
            ].map(([label, val]) => (
              <div key={label as string}>
                <div className="mb-1 flex justify-between text-indigo-100">
                  <span>{label}</span>
                  <span>{val}</span>
                </div>
                <div className="h-2 rounded bg-white/10">
                  <div
                    className="h-full rounded bg-gradient-to-r from-cyan-400 to-fuchsia-400"
                    style={{ width: `${val}%` }}
                  />
                </div>
              </div>
        ...
        <section className="relative z-10 mt-4 flex items-center justify-center md:mt-0">
          <div className="absolute h-64 w-64 rotate-6 border-2 border-cyan-300/60" />
          <div className="absolute h-64 w-64 -rotate-6 border-2 border-fuchsia-300/40" />
          {portrait ? (
            <img
              src={portrait}
              alt={girl.archetype}
              className="relative max-h-[65vh] w-auto rounded-xl object-cover drop-shadow-[0_0_40px_rgba(96,165,250,0.8)]"
            />
          ) : (
            <div className="h-[420px] w-[280px] rounded-xl bg-gradient-to-b from-fuchsia-500/40 to-cyan-400/30" />
          )}
        </section>
