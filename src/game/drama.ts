...
export function rollDrama(girls: Girl[], nowAbs: number): DramaResult | null {
  ...
  switch (type) {
        ...
        girls: girls.map((g) =>
          g.id === a.id || g.id === b.id
            ? {
                ...
                popularity: Math.min(99, g.popularity + 2),
              }
            : g,
        ),
        cashDelta: 0,
    ...
    case "jealousy": {
      ...
      return {
        girls: girls.map((g) =>
          g.id === a.id ? { ...g, loyalty: Math.min(99, g.loyalty + 4) } : g,
        ),
        cashDelta: -raise,
        repDelta: 0,
        heatDelta: 0,
      ...
      return {
        girls: girls.map((g) =>
          g.id === a.id
            ? {
                ...g,
                popularity: Math.min(99, g.popularity + 8),
    ...
    case "romance": {
      return {
        girls: girls.map((g) =>
          g.id === a.id || g.id === b.id ? { ...g, loyalty: Math.min(99, g.loyalty + 8) } : g,
        ),
        cashDelta: 0,
        repDelta: 2,
        heatDelta: 0,
    ...
    case "paparazzi": {
      ...
      return {
        girls: girls.map((g) =>
          g.id === a.id ? { ...g, popularity: Math.min(99, g.popularity + 5) } : g,
        ),
        cashDelta: $,
        repDelta: 1,
        heatDelta: 0,
      ...
      return {
        girls: girls.map((g) =>
          g.id === a.id
            ? {
                ...g,
                busyUntil: Math.max(g.busyUntil ?? 0, nowAbs + restHours),
                performance: Math.max(10, g.performance - 5),
              }
            : g,
        ),
        cashDelta: 0,
